...
class PrimaryIndex {
  private:
    ...
    std::unordered_map<std::string, std::vector<int>> SearchArray;
    int canUseSize;
    std::set<std::string> sender;
    void PrintPrimaryIndex();

  public:
    void RangeSearch (std::string fileNo);
    void EstablishPrimaryIndex (std::string fileNo, int useSize);
    void Sort(std::vector<Edge>& receiver, int size);
    void FindID(std::string ID, std::string fileNo);
    void IndexToData(int index, std::vector<Edge> &receiver, std::string fileNo);
...
int main() {
    ...
    do {
        ...
        while (std::cin >> temp) {
            if (IsInt(temp)) {
                canUseSize = stoi(temp);
                if (canUseSize > 60000 || canUseSize < 300) {
        ...
        externalSorter.ExternalSort(canUseSize);
        ...
        do {
            ...
            do {
                ...
                primaryIndex.FindID(temp1, fileNo);
            } while (temp1 != "4");
...
void PrintMenu() {
    ...
    std::cout << "* 3: Range search to build index *" << std::endl;
    std::cout << "* 4: Get maximum spanning forest *" << std::endl;
...
bool IsDouble(std::string coefficient) {
  ...
  for (int i = 0; i < coefficient.size(); i++) {
    if ((coefficient[i] > '9' || coefficient[i] < '0') && coefficient[i] != '.' && coefficient[i] != '+' && coefficient[i] != '-') {
    ...
    else if ((coefficient[i] == '+' || coefficient[i] == '-') && i != 0) {
      return false;
    }
...
void ExternalSorter::InnerSort(std::string inputFile, std::vector<std::string>& runFiles) {
    ...
    std::vector<Edge> buffer(canUseSize);
    int runCount = 0;

    while (fin) {
        fin.read(reinterpret_cast<char*>(buffer.data()), canUseSize * sizeof(Edge));//讀300筆Edge到buffer裡
...
void ExternalSorter::mergeTwoFiles(std::string file1, std::string file2, std::string outFile) {
    ...
    int size = canUseSize / 3;
    std::vector<Edge> buffer1(size);
    std::vector<Edge> buffer2(size);
    std::vector<Edge> outBuffer(size);

    int size1 = 0, size2 = 0;
    int pos1 = 0, pos2 = 0, posOut = 0;

    // file1 file2都先取一些資料出來
    fin1.read((char*)buffer1.data(), size * sizeof(Edge));
    size1 = fin1.gcount() / sizeof(Edge);
    fin2.read((char*)buffer2.data(), size * sizeof(Edge));
    ...
    while (size1 > 0 && size2 > 0) { //兩個資料都還有
        ...
        // buffer滿了再一次寫出
        if (posOut == size) {
            fout.write((char*)outBuffer.data(), size * sizeof(Edge));
            outBuffer.clear();
            posOut = 0;
        }
    }
...
void ExternalSorter::ExternalSort(int useSize) {
    canUseSize = useSize;
...
void PrimaryIndex::EstablishPrimaryIndex (std::string fileNo, int useSize) {
    canUseSize = useSize;
    std::string orderFile = "order" + fileNo + ".bin";
    std::ifstream fin(orderFile, std::ios::binary);
    primaryIndex.clear();
    std::vector<Edge> buffer(canUseSize);// 減少讀取次數
    float preWeight = -1;
    int offset = 0;
    while (true) {
        fin.read((char*)(buffer.data()), canUseSize*sizeof(Edge));
...
void PrimaryIndex::Sort(std::vector<Edge>& receiver, int size) {
    std::sort(receiver.begin() , receiver.begin() + size, [](Edge& a,Edge& b) { return a.weight > b.weight;});
}

void PrimaryIndex::FindID(std::string ID, std::string fileNo) {
    std::vector<int> searchIndex;
    std::vector<Edge> receiver;
    while (SearchArray.find(ID) == SearchArray.end()) { // 找不到這個發訊者
        std::cout << "Sender" << ID << "does not exist.\n";
        std::cout << "Input a student ID ([4] Quit): \n";
        std::cin >> ID;
        if (ID =="4") return;
    }
    
    searchIndex = SearchArray[ID];
    
    
    for (int i = 0; i < searchIndex.size(); i++) {
        IndexToData(searchIndex[i], receiver, fileNo);
    }

    int size = receiver.size();
    Sort(receiver, size);
    std::cout << "Sender" <<  ID << "has" <<  size << "records.\n";
    for (int i = 0; i < size; i++) {
        std ::cout << "[" << std::setw(3)<< i + 1 << "]";
        std::cout << std::setw(11) << receiver[i].getID;
        std::cout << std::setw(11) << receiver[i].weight << std::endl;
    }
}

void PrimaryIndex::IndexToData(int index, std::vector<Edge> &receiver, std::string fileNo) {
    std::string orderFile = "order" + fileNo + ".bin";
    std::ifstream fin(orderFile, std::ios::binary);
    fin.seekg(index * sizeof(Edge));
    Edge edge;
    fin.read(reinterpret_cast<char*>(&edge), sizeof(Edge));
    receiver.push_back(edge);
}

void PrimaryIndex::RangeSearch (std::string fileNo) {
    SearchArray.clear();
    sender.clear();
    ...
    while (std::cin >> temp) {
        if (IsDouble(temp)) {
            range1 = stof(temp);
            if (range1 > 1 || range1 < (0.01f - 0.00001f)) {
    ...
    while (std::cin >> temp) {
        if (IsDouble(temp)) {
            range2 = stof(temp);
            if (range2 > 1 || range2 <  (0.01f - 0.00001f)) {
                std::cout << "### It is NOT in [0.01,1] ###\n\n";
            } else break;
        }
        std::cout << "Input a floating number in [0.01, 1]: \n\n";
    }
    if (range1 < range2) std::swap(range1, range2);//range1大
    ...
    for (int i = 0; i < primaryIndex.size(); i++) {
        if (primaryIndex[i].weight <= range1 && primaryIndex[i].weight >= range2) {
            if (indexStart == -1) indexStart = i; // 第一個是最大值
            indexEnd = i;//最後的就是最小值
        }
    }

    int startOffset = primaryIndex[indexStart].offset;
    int endOffset = 0;

    fin.seekg(0, std::ios::end);
    int totalRecords = fin.tellg() / sizeof(Edge);

    if (indexEnd == primaryIndex.size() - 1) endOffset = totalRecords - 1;
    else endOffset = primaryIndex[indexEnd + 1].offset - 1;

    int totalNeedRead = endOffset - startOffset + 1;
    int records = totalNeedRead;

    fin.seekg(startOffset * sizeof(Edge), std::ios::beg);
    std::vector<Edge> buffer(canUseSize);
    int currentOffset = startOffset;

    while (totalNeedRead > 0) {
        int readSize = std::min(canUseSize, totalNeedRead);

        fin.read(reinterpret_cast<char*>(buffer.data()), readSize * sizeof(Edge));
        int realSize = fin.gcount() / sizeof(Edge);

        for (int i = 0; i < realSize; i++) {
            std::string putID(buffer[i].putID);
            SearchArray[putID].push_back(currentOffset);
            sender.insert(putID);
            currentOffset++;
        }

        totalNeedRead -= realSize;
    }

    fin.close();

    int num = 1;
    std::cout << "There are " << records << " records in total.\n";
    std::cout << "There are " << sender.size() << " senders in total.\n";
    for (const std::string &id : sender) {//set一定要用const
        std::cout << "[" << std::setw(4) << num++ << "]" << std::setw(11) << id << "\t" << std::setw(5) << SearchArray[id].size()<< "\n";
    }
}
